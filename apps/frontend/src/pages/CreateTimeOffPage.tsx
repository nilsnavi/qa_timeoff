import { useMutation, useQueryClient } from '@tanstack/react-query';
import { CheckCircle2, Clock3 } from 'lucide-react';
import { useEffect, useMemo, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { Button, Card, DatePicker, EmptyState, Input, Textarea } from '../components/ui';
import { api } from '../shared/api';
import { useDashboard } from '../shared/hooks/useDashboard';
import { toDateInputValue } from '../shared/utils/date';

type FormErrors = Partial<Record<'date' | 'hours' | 'reason' | 'comment', string>>;

export function CreateTimeOffPage() {
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const { dashboard } = useDashboard();
  const availableHours = dashboard.balance.balanceHours;
  const [date, setDate] = useState(toDateInputValue());
  const [hours, setHours] = useState(8);
  const [reason, setReason] = useState('Time off');
  const [comment, setComment] = useState('');
  const [errors, setErrors] = useState<FormErrors>({});
  const [isSuccess, setIsSuccess] = useState(false);

  const remainingAfterRequest = useMemo(() => Math.max(availableHours - (Number.isFinite(hours) ? hours : 0), 0), [availableHours, hours]);

  const mutation = useMutation({
    mutationFn: api.createTimeOff,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['dashboard'] });
      setIsSuccess(true);
    },
  });

  useEffect(() => {
    if (!isSuccess) {
      return;
    }

    const timeout = window.setTimeout(() => navigate('/requests'), 1300);
    return () => window.clearTimeout(timeout);
  }, [isSuccess, navigate]);

  const submit = () => {
    const nextErrors = validate({ date, hours, reason, comment, availableHours });
    setErrors(nextErrors);

    if (Object.keys(nextErrors).length > 0) {
      return;
    }

    mutation.mutate({ date, hours, reason, comment: comment || undefined });
  };

  if (isSuccess) {
    return (
      <EmptyState
        title="Request created"
        description="Sending you to requests"
        action={<CheckCircle2 className="mx-auto text-emerald-500" size={32} />}
      />
    );
  }

  return (
    <>
      <Card className="app-gradient text-white">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-sm font-bold opacity-80">Available</p>
            <p className="mt-1 text-4xl font-black">{availableHours} h</p>
          </div>
          <Clock3 size={34} />
        </div>
      </Card>

      <Card>
        <h2 className="mb-4 text-lg font-black text-slate-950 dark:text-white">Create time off</h2>
        <div className="grid gap-4">
          <DatePicker label="Date" value={date} error={errors.date} onChange={(event) => setDate(event.target.value)} />
          <Input
            label="Hours"
            type="number"
            min={1}
            max={availableHours}
            value={hours}
            error={errors.hours}
            hint={`Remaining after request: ${remainingAfterRequest} h`}
            onChange={(event) => setHours(Number(event.target.value))}
          />
          <Input label="Reason" value={reason} error={errors.reason} onChange={(event) => setReason(event.target.value)} />
          <Textarea
            label="Comment"
            value={comment}
            maxLength={500}
            error={errors.comment}
            hint={`${comment.length}/500`}
            onChange={(event) => setComment(event.target.value)}
          />
          {mutation.isError && <p className="text-sm font-bold text-rose-500">Could not create request. Try again.</p>}
          <Button size="lg" onClick={submit} disabled={mutation.isPending}>
            {mutation.isPending ? 'Creating' : 'Create request'}
          </Button>
        </div>
      </Card>
    </>
  );
}

function validate({
  date,
  hours,
  reason,
  comment,
  availableHours,
}: {
  date: string;
  hours: number;
  reason: string;
  comment: string;
  availableHours: number;
}) {
  const errors: FormErrors = {};

  if (!date) {
    errors.date = 'Date is required';
  }

  if (!hours || hours <= 0) {
    errors.hours = 'Hours are required';
  } else if (hours > availableHours) {
    errors.hours = 'Cannot request more than available balance';
  }

  if (!reason.trim()) {
    errors.reason = 'Reason is required';
  }

  if (comment.length > 500) {
    errors.comment = 'Comment must be 500 characters or less';
  }

  return errors;
}
