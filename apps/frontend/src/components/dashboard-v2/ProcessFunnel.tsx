...
export function ProcessFunnel({ dashboard }: { dashboard: Dashboard }) {
  ...
  return (
    <div className="enterprise-card p-5">
      <p className="text-[11px] font-bold uppercase tracking-widest text-white/25 mb-4">Воронка заявок</p>
        ...
        {stages.map((stage) => {
          ...
          return (
            <div key={stage.label} className="flex items-center gap-3">
              <span className="w-24 shrink-0 text-[12px] font-semibold text-white/50">{stage.label}</span>
              <div className="flex-1 h-7 rounded-md bg-white/[0.03] overflow-hidden">
                <div
                  className={`h-full rounded-md ${stage.bg} ${stage.color} flex items-center px-3 text-[11px] font-bold transition-all duration-500`}
                  style={{ width: `${width}%`, minWidth: count > 0 ? '40px' : '0' }}
                >
                  {count > 0 && count}
                </div>
              </div>
              <span className="w-8 text-right text-[12px] font-bold text-white/30">{count}</span>
            </div>
          );
        })}
      </div>
