  ...
  return (
    <div className="enterprise-card p-5 hover-lift">
      <div className="flex items-center justify-between mb-4">
        <p className="text-[11px] font-bold uppercase tracking-widest text-white/25">Календарь</p>
        <div className="flex items-center gap-1">
          ...
          <span className="min-w-[110px] text-center text-[12px] font-semibold text-white/70">{MONTHS[month]} {year}</span>
      ...
      <div className="grid grid-cols-7 mb-1">
        {DAYS_SHORT.map(d => <span key={d} className="text-center text-[9px] font-bold text-white/20 py-1">{d}</span>)}
      </div>
      ...
      <div className="grid grid-cols-7">
        ...
        {Array.from({ length: daysInMonth }).map((_, i) => {
          ...
          return (
            ...
            >
              <span className={`text-[11px] font-semibold rounded-md w-7 h-7 flex items-center justify-center ${isToday ? 'bg-[#4C7DFF]/20 text-[#4C7DFF]' : 'text-white/50'}`}>{day}</span>
