...
export function TeamWorkload({ dashboard }: { dashboard: Dashboard }) {
  ...
  return (
    <div className="enterprise-card p-5">
      ...
      {members.length === 0 ? (
        <p className="text-[13px] text-white/30 py-4 text-center">Нет данных о команде</p>
      ) : (
            ...
            return (
              <div key={member.name} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-6 rounded-md bg-white/[0.03] overflow-hidden flex">
                  ...
                  {member.pending > 0 && (
                    <div className="h-full bg-amber-500/20 text-amber-400 flex items-center justify-center text-[10px] font-bold transition-all"
      ...
      <div className="flex items-center gap-4 mt-4 pt-3 border-t border-white/[0.04]">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-white/25">Одобрено</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="h-2 w-2 rounded-sm bg-amber-500/40" />
          <span className="text-[10px] text-white/25">Ожидает</span>
        </div>
      </div>
    </div>
  );
